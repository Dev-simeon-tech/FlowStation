...
export default function Stock() {
  useEffect(() => {
    document.title = "Stock / FlowStation";
  }, []);
  ...
  function loadAll(signal?: AbortSignal) {
    ...
    Promise.all([
      ...
      apiFetch<FuelStock[]>("/api/stock"),
      apiFetch<StockBalance[]>("/api/stock/balance"),
    ])
      .then(([prods, sups, stockData, stockBalance]) => {
        if (signal?.aborted) return;
        setProducts(prods);
        setSuppliers(sups);
        setStocks(stockData);
        setBalances(stockBalance);
      })
      .catch((e) => {
  ...
  const totalStock = balances.reduce((sum, b) => sum + b.currentBalance, 0);
    ...
    <div>
      <div className={s.topBar}>
        ...
        <div className={s.tabs}>
          <button
            className={`${s.tab} ${tab === "balance" ? s.tabActive : ""}`}
            onClick={() => setTab("balance")}
          >
            Balance
          </button>
          <button
            className={`${s.tab} ${tab === "entry" ? s.tabActive : ""}`}
        ...
        <>
          <div className={s.card}>
            ...
            {loading ? (
              ...
              <div className={s.empty}>
                <p>No stock on record. Record a delivery first.</p>
              </div>
            ) : (
                ...
                <tbody>
                  {balances.map((b) => (
                    <tr key={b.fuelProductId}>
                      <td>{b.fuelProductName}</td>
                      <td>
                        <span
                          className={`${s.badge} ${s["badge" + b.fuelType] ?? s.badgeInfo}`}
                        >
                          {b.fuelType}
                        </span>
                      </td>
                      ...
                      <td>
                        <strong>{b.currentBalance.toLocaleString()}</strong>
                      </td>
                    </tr>
                  ))}
                </tbody>
                ...
                <tbody>
                  {stocks.map((r) => (
                    <tr key={r.id}>
                      ...
                      <td>
                        &#8358;
                        {(r.quantityLitres * r.costPerLitre).toLocaleString()}
                      </td>
        ...
        <div className={s.card}>
          ...
          <div className={s.formGrid}>
            ...
            <NumberInput
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, quantityLitres: e.target.value }))
              }
            />
            <NumberInput
              ...
              onChange={(e) =>
                setForm((f) => ({ ...f, costPerLitre: e.target.value }))
              }
            />
            <div className={s.formGroup}>
