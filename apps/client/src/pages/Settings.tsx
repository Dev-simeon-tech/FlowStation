...
export default function Settings() {
  ...
  useEffect(() => {
    if (user) {
      if (user.org) {
        const nameVal = user.org.name;
        const emailVal = user.org.email;
        let addressVal = "";
        let logoVal = "";
  ...
  useEffect(() => {
    if (activeTab === "system") {
      ...
      apiFetch<SystemConfig>("/api/settings")
        ...
        .catch((e) => {
          console.warn(
            "Could not fetch settings from API, using local fallback:",
            e.message,
          );
        })
        .finally(() => {
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    reader.onload = () => {
      if (typeof reader.result === "string") {
        setOrgForm((prev) => {
          const updated = { ...prev, logo: reader.result as string };
          return updated;
        });
      }
    };
  ...
  async function handleProfileSubmit(e: React.FormEvent) {
    ...
    try {
      await apiFetch<{ message: string; org: typeof orgForm }>(
        "/api/organization",
        {
          method: "PUT",
      ...
      let errMsg =
        "Failed to update profile. Server code may be missing, local preview updated.";
      if (err.message) {
        errMsg = err.message;
      }
      setError(errMsg);
  ...
  if (success) {
    successDisplay = (
      <p
        ...
        style={{ marginBottom: 16, color: "var(--success)" }}
      >
        {success}
      </p>
    );
  ...
  if (activeTab === "profile") {
          ...
          <div className={s.formGrid}>
            <div className={s.formGroup}>
              ...
              <select
                ...
                onChange={(e) =>
                  setSysForm((prev) => {
                    return { ...prev, currencySymbol: e.target.value };
                  })
                }
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    setSysForm((prev) => {
                      return {
                        ...prev,
                        requireShiftHandover: e.target.checked,
                      };
                    })
                  }
                />
    ...
    tabBodyContent = <div className={s.card}>{settingsContent}</div>;
  }
    ...
    <div>
      <div className={s.topBar}>
        ...
        <div className={s.tabs}>
          <button className={profileTabClass} onClick={() => setTab("profile")}>
            Organization Profile
          </button>
          <button className={systemTabClass} onClick={() => setTab("system")}>
            System Settings
          </button>
        </div>
      </div>
